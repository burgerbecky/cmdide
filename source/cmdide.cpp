/***************************************
	...
	Tool to invoke CodeWarrior's IDE for automatic building and project
	management

	Note: This uses Carbon libraries that are not available in 64 bit CPUs, so
	only compile for 32 bit CPUs

	Inspired by a tool of the same name by Josef W. Wankerl of Metrowerks
	Corporation
...
static const uint8_t g_BooleanTrue = TRUE;
static const uint32_t g_NoYes[2] = {'no  ', 'yes '};
...
static void PrintErrorsOrWarnings(
	const AEDesc* pErrorList, uint_t bWarnings, FILE* fp)
{
	long mItemCount;
	if (!AECountItems(pErrorList, &mItemCount)) {
		...
		if (lItemCount) {
			...
			do {
				...
				AEDesc AEResult;
				if (AEGetNthDesc(
						pErrorList, lIndex, 'ErrM', &MyKeyWord, &AEResult)) {
					break;
				}
				// Get the error type
				Size lActualSize;
				DescType MyDescType;
				uint32_t eErrorType;
				if (!AEGetParamPtr(&AEResult, 'ErrT', 'enum', &MyDescType,
						&eErrorType, sizeof(eErrorType), &lActualSize)) {
					...
					} else if ((eErrorType == 'ErCE') ||
						(eErrorType == 'ErLE') || (eErrorType == 'ErGn')) {
						bValid = TRUE;
					}
					...
					if (bValid) {
						...
						if (!AEGetParamPtr(&AEResult, 'ErrS', 'TEXT',
								&MyDescType, &TextBuffer,
								sizeof(TextBuffer) - 1, &lActualSize)) {
							// Force to a "C" string
							...
							bValid = FALSE;
							FSSpec MyFileSpec;
							if (!AEGetParamPtr(&AEResult, 'file', 'fss ',
									&MyDescType, &MyFileSpec,
									sizeof(MyFileSpec), &lActualSize)) {
								bValid = TRUE;
								// Print the filename
							...
							long lLineNumber;
							if (!AEGetParamPtr(&AEResult, 'ErrL', 'long',
									&MyDescType, &lLineNumber,
									sizeof(lLineNumber), &lActualSize)) {
								bValid = TRUE;
...
static uint_t IsThereAnyItems(const AEDesc* pAEDesc)
{
	long mItemCount;
	uint_t bResult;
...
static int IsCodeWarriorRunning(
	const char* pIDEName, ProcessSerialNumber* pProcess)
{
	...
	MyProcessInfo.processName = nullptr;

	int iError;

	// If there is no supplied name, just find the first app with the
	// CodeWarrior signature

	if (!pIDEName) {
		// I'm not interested in the FSSpec
		MyProcessInfo.processAppSpec = nullptr;
		...
		// First, take the filename and convert to an FSSpec so I can scan for a
		// match
		iError = FSPathMakeRef(
			reinterpret_cast<const UInt8*>(pIDEName), &MyRef, &bDirectory);
		if (!iError) {
			FSSpec MySpec;
			// Convert to an FSSpec
			iError = FSGetCatalogInfo(
				&MyRef, kFSCatInfoNone, nullptr, nullptr, &MySpec, nullptr);
			if (!iError) {
				...
				do {
					...
					MemoryCompare(
						&MySpec.name[1], &ProcessSpec.name[1], MySpec.name[0]));
			}
		}
	}
	return iError;
...
static int FindOrLaunchCodeWarrior(
	const char* pIDEName, ProcessSerialNumber* pProcess)
{
	...
	if (iResult) {
		...
		if (!pIDEName) {
			...
			iResult = LSGetApplicationForInfo(
				kLSUnknownType, 'CWIE', nullptr, kLSRolesAll, &MyRef, nullptr);
		} else {
			// Launch from a specific pathname
			Boolean bDirectory;
			iResult = FSPathMakeRef(
				reinterpret_cast<const UInt8*>(pIDEName), &MyRef, &bDirectory);
		}
		// Did I find the file reference?
		if (!iResult) {
			// Start up the application!
			iResult = LSOpenFSRef(&MyRef, nullptr);
			if (!iResult) {
...
static int CreateAppleEventForCodeWarrior(AEDesc* pDesc, uint32_t uEventClass,
	uint32_t uEventID, const char* pIDEName)
{
	...
	if (!iResult) {
		...
		if (!pIDEName) {
			...
			uint32_t uSignature = 'CWIE';
			iResult = AECreateDesc(
				typeApplSignature, &uSignature, sizeof(uSignature), &MyDesc);
		} else {
			// Send to the specific ID
			iResult = AECreateDesc(typeProcessSerialNumber, &MyProcessNumber,
				sizeof(MyProcessNumber), &MyDesc);
		}
		// No problem making the event?
		if (!iResult) {
			// Let's send the event to CodeWarrior
			iResult = AECreateAppleEvent(uEventClass, uEventID, &MyDesc,
				kAutoGenerateReturnID, kAnyTransactionID, pDesc);
...
static int BuildTarget(AEDesc* pDesc, const char* pIDEName)
{
	...
	int iError =
		CreateAppleEventForCodeWarrior(&BuildEvent, 'MMPR', 'Make', pIDEName);
	if (!iError) {
		// Add the message parameter 'Errs' = TRUE to get the error list
		iError = AEPutParamPtr(
			&BuildEvent, 'Errs', 'bool', &g_BooleanTrue, sizeof(g_BooleanTrue));
		if (!iError) {
			AEDesc MyReply;
			AEInitializeDesc(&MyReply);
			// Tell CodeWarrior to build the project
			iError = AESend(&BuildEvent, &MyReply,
				kAEDontRecord + kAECanInteract + kAEQueueReply + kAENoReply,
				kAENormalPriority, kNoTimeOut, nullptr, nullptr);
			if (!iError) {
...
static int CloseProject(const char* pIDEName, int iTimeout)
{
	...
	int iError =
		CreateAppleEventForCodeWarrior(&CloseEvent, 'MMPR', 'ClsP', pIDEName);
	if (!iError) {
		AEDesc MyReply;
		AEInitializeDesc(&MyReply);
		iError = AESend(&CloseEvent, &MyReply,
			kAEDontRecord + kAECanInteract + kAEQueueReply + kAENoReply,
			kAENormalPriority, iTimeout, nullptr, nullptr);
		if (!iError) {
			AEDisposeDesc(&MyReply);
		}
...
static int ShutDownCodeWarrior(const char* pIDEName, int iTimeOut)
{
	...
	int iError =
		CreateAppleEventForCodeWarrior(&QuitEvent, 'aevt', 'quit', pIDEName);
	if (!iError) {
		AEDesc MyReply;
		AEInitializeDesc(&MyReply);
		iError = AESend(&QuitEvent, &MyReply,
			kAEDontRecord + kAECanInteract + kAEQueueReply + kAENoReply,
			kAENormalPriority, iTimeOut, nullptr, nullptr);
		if (!iError) {
			AEDisposeDesc(&MyReply);
...
static int CleanProject(const char* pIDEName, int iTimeOut)
{
	...
	int iError =
		CreateAppleEventForCodeWarrior(&CleanEvent, 'MMPR', 'RemB', pIDEName);
	if (!iError) {
		AEDesc MyReply;
		AEInitializeDesc(&MyReply);
		iError = AESend(&CleanEvent, &MyReply,
			kAEDontRecord + kAECanInteract + kAEQueueReply + kAENoReply,
			kAENormalPriority, iTimeOut, nullptr, nullptr);
		if (!iError) {
			AEDisposeDesc(&MyReply);
...
static int SetBuildTarget(
	const char* pTargetName, const char* pIDEName, int iTimeOut)
{
	AEDesc SetProjectEvent;
	int iError = CreateAppleEventForCodeWarrior(
		&SetProjectEvent, 'MMPR', 'STrg', pIDEName);
	if (!iError) {
		// Set the target name in the parameter
		iError = AEPutParamPtr(&SetProjectEvent, '----', 'TEXT', pTargetName,
			(long)StringLength(pTargetName));
		if (!iError) {
			AEDesc MyReply;
			AEInitializeDesc(&MyReply);
			iError = AESend(&SetProjectEvent, &MyReply,
				kAEDontRecord + kAECanInteract + kAEQueueReply + kAENoReply,
				kAENormalPriority, iTimeOut, nullptr, nullptr);
			if (!iError) {
				AEDisposeDesc(&MyReply);
...
static int OpenProjectFile(const char* pFilename, const char* pIDEName,
	uint_t bAllowConversion, int iTimeout)
{
	FSRef MyRef;
	Boolean bIsDirectory;
	int iError = FSPathMakeRef(
		reinterpret_cast<const UInt8*>(pFilename), &MyRef, &bIsDirectory);
	if (!iError) {
		if (bIsDirectory) {
			...
			AEDesc OpenDocEvent;
			iError = CreateAppleEventForCodeWarrior(
				&OpenDocEvent, 'aevt', 'odoc', pIDEName);
			if (!iError) {

				// The Generic open doc event has the ability to open multiple
				// files, which is why it needs a list instead of a single
				// parameter
				AEDescList MyDescList;
				iError = AECreateList(nullptr, 0, FALSE, &MyDescList);
				if (!iError) {
					// Add a FSRef to the list
					iError =
						AEPutPtr(&MyDescList, 0, 'fsrf', &MyRef, sizeof(MyRef));
					if (!iError) {

						// Insert the FSRef list to the "Open a Document" event
						iError =
							AEPutParamDesc(&OpenDocEvent, '----', &MyDescList);
						AEDisposeDesc(&MyDescList);
						if (!iError) {

							// Do I allow project conversion?
							const uint32_t* pConversionRequest = g_NoYes;
							if (bAllowConversion) {
								++pConversionRequest; // Switch to 'yes '
							}
							iError = AEPutParamPtr(&OpenDocEvent, 'Conv',
								'enum', pConversionRequest, sizeof(uint32_t));
							if (!iError) {
								// Create my reply description
								AEDesc MyReply;
								AEInitializeDesc(&MyReply);
								// Tell CodeWarrior to open the project file
								iError = AESend(&OpenDocEvent, &MyReply,
									kAEDontRecord + kAECanInteract +
										kAEQueueReply + kAENoReply,
									kAENormalPriority, iTimeout, nullptr,
									nullptr);
								if (!iError) {
									AEDisposeDesc(&MyReply);
								}
...
static int DiffFiles(const char* pIDEName, const char* pParm1,
	const char* pParm2, uint_t bIgnoreWhiteSpace, uint_t bIgnoreCase,
	uint_t bSkipSameFiles, uint_t bCompareContents, int iTimeout)
{
	Boolean bDirectory1;
	FSRef Ref1;
	int iError = FSPathMakeRef(
		reinterpret_cast<const UInt8*>(pParm1), &Ref1, &bDirectory1);
	if (iError) {
		fprintf(
			stderr, "error: cw diff: %s: no such file or directory\n", pParm1);
	} else {
		Boolean bDirectory2;
		FSRef Ref2;
		iError = FSPathMakeRef(
			reinterpret_cast<const UInt8*>(pParm2), &Ref2, &bDirectory2);
		if (iError) {
			fprintf(stderr, "error: cw diff: %s: no such file or directory\n",
				pParm2);
		} else if (bDirectory1 != bDirectory2) {
			fprintf(stderr,
				"error: cw diff: cannot compare a file with a directory\n");
			iError = -1;
		} else {
			...
			iError = CreateAppleEventForCodeWarrior(
				&DiffFileEvent, 'MMPR', 'CmpF', pIDEName);
			if (!iError) {
				// Add the parameters

				// First, the files to compare
				iError = AEPutParamPtr(
					&DiffFileEvent, '----', 'fsrf', &Ref1, sizeof(Ref1));
				if (!iError) {
					iError = AEPutParamPtr(
						&DiffFileEvent, 'Cmpt', 'fsrf', &Ref2, sizeof(Ref2));
					if (!iError) {

						// Now, the parameters
						Boolean bParameter =
							static_cast<Boolean>(bIgnoreWhiteSpace);
						iError = AEPutParamPtr(&DiffFileEvent, 'CmCS', 'bool',
							&bParameter, sizeof(bParameter));
						if (!iError) {
							bParameter = static_cast<Boolean>(bIgnoreCase);
							iError = AEPutParamPtr(&DiffFileEvent, 'CmIX',
								'bool', &bParameter, sizeof(bParameter));
							if (!iError) {
								bParameter =
									static_cast<Boolean>(bSkipSameFiles);
								iError = AEPutParamPtr(&DiffFileEvent, 'CmDF',
									'bool', &bParameter, sizeof(bParameter));
								if (!iError) {
									bParameter =
										static_cast<Boolean>(bCompareContents);
									iError = AEPutParamPtr(&DiffFileEvent,
										'CmTC', 'bool', &bParameter,
										sizeof(bParameter));
									if (!iError) {

										// Ready to go? Issue the "Diff Files"
										// event
										AEDesc MyReply;
										AEInitializeDesc(&MyReply);
										iError =
											AESend(&DiffFileEvent, &MyReply,
												kAEDontRecord + kAECanInteract +
													kAEQueueReply + kAENoReply,
												kAENormalPriority, iTimeout,
												nullptr, nullptr);
										if (!iError) {
											AEDisposeDesc(&MyReply);
										}
...
/***************************************
	...
	Act upon a project file. The operations must be done in the order of Open,
Set Target, Clean, Build, Close project, close CodeWarrior

***************************************/

static int PerformProjectOperations(const char* pFilename,
	const char* pTargetName, const char* pIDEName, uint_t bAllowConversion,
	uint_t bCleanProject, uint_t bBuildProject, uint_t bQuitCodeWarrior,
	uint_t bCloseProject, uint_t bShowWarnings, uint_t bShowErrors,
	const char* pErrorFile, int iTimeout)
{
	int iError =
		OpenProjectFile(pFilename, pIDEName, bAllowConversion, iTimeout);
	if (!iError) {
		if (pTargetName &&
			(iError = SetBuildTarget(pTargetName, pIDEName, iTimeout)) != 0) {
			fprintf(stderr, "error: could not change target\n");
		} else if (bCleanProject &&
			(iError = CleanProject(pIDEName, iTimeout)) != 0) {
				...
				if (pErrorFile) {
					if (IsThereAnyItems(&MyErrors)) {
						...
						if (!fp) {
							...
							bShowWarnings = FALSE;
							fprintf(stderr,
								"warning: could not open the errors and warnings output file\n");
						}
					}
				}
...
static int ProcessProjCommand(int argc, char** argv, int* pIndex)
{
	...
	const char* pIDEName = nullptr;
	const char* pErrorFile = nullptr;
	uint_t bAllowConversion = 0;
	int iTimeout = kAEDefaultTimeout;
	uint_t bCleanProject = 0;
	uint_t bBuildProject = 0;
	uint_t bQuitCodeWarrior = 0;
	uint_t bCloseProject = 0;
	uint_t bShowWarnings = 0;
	uint_t bShowErrors = 0;
	int iIndex = *pIndex;
	for (; iIndex < argc; ++iIndex) {
		...
		if ((pArg[0] == '-') && pArg[1]) {
			...
			size_t i;
			uintptr_t MaxLen = StringLength(pArg);
			for (i = 1; i < MaxLen; ++i) {
				switch (pArg[i]) {
				case 'x':
					bAllowConversion = 1;
				...
				case 'f':
					if ((++iIndex) >= argc) {
						fprintf(stderr,
							"warning: no errors and warnings output filename specified\n");
					} else {
			...
			// Is this a valid project file argument? (More than one letter
			// long)?
			if (pArg[1]) {
	...
	if (iIndex >= argc) {
		...
		iError = PerformProjectOperations(argv[iIndex], pTargetName, pIDEName,
			bAllowConversion, bCleanProject, bBuildProject, bQuitCodeWarrior,
...
{
	uint_t bSkipSameFiles = TRUE;
	uint_t bIgnoreCase = FALSE;
	uint_t bIgnoreWhiteSpace = FALSE;
	const char* pIDEName = nullptr;
	int iTimeout = kAEDefaultTimeout;
	int iIndex = *pIndex;
	for (; iIndex < argc; ++iIndex) {
		...
		if (pArg[0] == '-' && pArg[1]) {
			...
			fprintf(stderr,
				"notice: cw diff: ignoring project argument \"%s\"\n", pArg);
		}
	}
	// Perform the diff
	int iResult;
	if (iIndex >= argc) {
		...
		iResult = DiffFiles(pIDEName, argv[iIndex], argv[iIndex + 1],
			bIgnoreWhiteSpace, bIgnoreCase, bSkipSameFiles, TRUE, iTimeout);
		iIndex += 2;
	}
	*pIndex = iIndex;
	return iResult;
